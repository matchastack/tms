...
docker save -o node-22-alpine.tar node:22-alpine

# generate checksum to manifest.txt

sha256sum server-1.0.0.tgz node-22-alpine.tar > manifest.txt

# compress and password protect each file

tar -czf - server-1.0.0.tgz node-22-alpine.tar | gpg --symmetric --cipher AES256 -o transfer-bundle.tar.gz.gpg
...
docker run --env-file .env -p 8080:8080 --name tms -d tms-micro-eval
...
docker run --env-file ../../devenv/project/.env -p 8080:8080 --name tms-uat -d tms-micro-eval
